new Vue({
  ...
  mounted:function(){
    ...
    this.setIntervalTime = setInterval(this.timeDate,1000);
    this.loveSetIntervalTime = setInterval(this.loveTime,1000);
  ...
  methods:{
    ...
    timeDate:function(){
      ...
      this.timeLabel = days+'天,'+hours+'小时,'+minutes+"分钟,"+seconds+'秒'
    },
    // 时间计算2
    loveTime: function(){
      var nowTime = new Date().getTime();
      var nowTimeStr = parseInt(nowTime / 1000);
      var newTime = '2018-11-24 0:00:00';
      var newTimeStr = parseInt(new Date(newTime).getTime() / 1000);
      ...
      // 相差秒数
      var seconds = Math.round(distanceTime % 60);
      this.lovetTimeLabel = days+'天,'+hours+'小时,'+minutes+"分钟,"+seconds+'秒'
    },
    // 场景值1
